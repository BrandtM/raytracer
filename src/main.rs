...
fn main() {
	...
	let main_sphere = Sphere {center: Vector3::new(0.0, 0.0, -1.0), radius: 0.5};
	let ground_sphere = Sphere {center: Vector3::new(0.0, -100.5, -1.0), radius: 100.0};
	...
	let pixels: Vec<Vec<Pixel>> = (0..y_resolution).into_par_iter().map(|y| {
		(0..x_resolution).into_par_iter().map(|x| {
			let mut color = (0..samples_per_pixel).into_par_iter().map(|_| {
				...
				ray.color(hitable_list.clone())
			}).reduce(|| Vector3::new(0.0, 0.0, 0.0), |col, ray_color| col + ray_color);
			
			color /= samples_per_pixel as f32;

			Pixel {
				red: (255_f32 * color.x) as u8,
				green: (255_f32 * color.y) as u8,
				blue: (255_f32 * color.z) as u8
			}
		}).collect()
	}).collect();
